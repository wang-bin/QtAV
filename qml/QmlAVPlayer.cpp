...
void QmlAVPlayer::classBegin()
{
    ...
    connect( mpPlayer, SIGNAL( internalSubtitleTracksChanged( QVariantList ) ),
             SIGNAL( internalSubtitleTracksChanged() ) );
    connect( mpPlayer, SIGNAL( internalAudioTracksChanged( QVariantList ) ),
             SIGNAL( internalAudioTracksChanged() ) );
    connect( mpPlayer, SIGNAL( internalVideoTracksChanged( QVariantList ) ),
             SIGNAL( internalVideoTracksChanged() ) );
    connect( mpPlayer, SIGNAL( externalAudioTracksChanged( QVariantList ) ),
             SIGNAL( externalAudioTracksChanged() ) );
    ...
    connect( mpPlayer, SIGNAL( seekFinished( qint64 ) ), this, SIGNAL( seekFinished() ),
             Qt::DirectConnection );
    ...
    connect( mpPlayer->audio(), SIGNAL( volumeChanged( qreal ) ), SLOT( applyVolume() ),
             Qt::DirectConnection );
    connect( mpPlayer->audio(), SIGNAL( muteChanged( bool ) ), SLOT( applyVolume() ),
             Qt::DirectConnection );
...
void QmlAVPlayer::setSource( const QUrl& url )
{
    ...
    if( m_complete && ( mAutoLoad || mAutoPlay ) )
    {
        ...
        if( mAutoPlay )
        {
            // mPlaybackState is actually changed in slots. But if set to a new source
            // the state may not change before call play()
            mPlaybackState = StoppedState;
            play();
        }
    }
...
// mVolume, m_mute are required by qml properties. player.audio()->setXXX is not
// enought because player maybe not created
...
void QmlAVPlayer::setPlaybackState( PlaybackState playbackState )
{
    ...
    case PlayingState:
        ...
        else
        {
            ...
            {
                QVariantHash vcopt;
                for( QVariantMap::const_iterator cit = vcodec_opt.cbegin(); cit != vcodec_opt.cend();
                     ++cit )
                {
                    vcopt[cit.key()] = cit.value();
                }
            ...
            {
                QVariantHash avfopt;
                for( QVariantMap::const_iterator cit = avfmt_opt.cbegin(); cit != avfmt_opt.cend();
                     ++cit )
                {
                    avfopt[cit.key()] = cit.value();
                }
