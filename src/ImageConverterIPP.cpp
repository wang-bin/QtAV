...
bool ImageConverterIPP::convert(const quint8 *const srcSlice[], const int srcStride[])
{
    ...
    ippiYUV420ToRGB_8u_P3AC4R(const_cast<const quint8 **>(srcSlice), const_cast<int*>(srcStride), (Ipp8u*)(d.data_out.data())
                           ,4*sizeof(quint8)*d.w_out, (IppiSize){d.w_in, d.h_in});
    return true;
}
