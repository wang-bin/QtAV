    ...
    : nb_tex(texCount)
{
    if (texCount < 1)
        texCount = 1;
    v.resize(nb_tex*textureVertexCount());
    a = QVector<Attribute>()
...
void TexturedGeometry::setTextureCount(int value)
{
    ...
    if (a.size()-1 < value) { // the first is position
        for (int i = a.size()-1; i < value; ++i)
            a << Attribute(GL_FLOAT, 2, i*textureSize() + 2*sizeof(float));
    } else {
        a.resize(value + 1);
    }
}
...
{
    v[texIndex*textureVertexCount() + index].x = p.x();
    v[texIndex*textureVertexCount() + index].y = p.y();
}

void TexturedGeometry::setTexturePoint(int index, const QPointF &tp, int texIndex)
{
    v[texIndex*textureVertexCount() + index].tx = tp.x();
    v[texIndex*textureVertexCount() + index].ty = tp.y();
}
