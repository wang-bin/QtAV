...
int getEncOpenParam(EncOpenParam *pEncOP, EncConfigParam *pEncConfig, ENC_CFG *pEncCfg)
{
    ...
    case STD_AVC:
        ...
        if (pEncCfg)
            ...
            sprintf(pEncConfig->yuvFileName,  "%s%s", "\\\\52.2.12.200\\AllFiles\\qctool\\img\\", pCfg->SrcFileName);		
        break;
    case STD_MPEG4:
    case STD_H263:
    ...
    pEncOP->enableAutoSkip = 1;//pCfg->enableAutoSkip;
...
int parseAvcCfgFile(ENC_CFG *pEncCfg, char *FileName)
{
    ...
    */
    ...
    if (GetValue(fp, "FRAME_SKIP_DISABLE", sValue) == 0)
        pEncCfg->enableAutoSkip = 0;
    else
        pEncCfg->enableAutoSkip = 1;
...
static AVStream *add_stream(AVFormatContext *oc, enum CodecID codec_id, EncConfigParam encConfig, int gopSize)
{
    AVCodecContext *c;
    AVStream *st;
